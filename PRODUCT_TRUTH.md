...
before enforcement; the ENTIRE Company Brain — the six scalar stores (marketing,
which grounds the governance observe chain, plus creative, SOP, sales, DNA, brand)
AND the experience / pattern-library / knowledge-graph sub-brains — is now DURABLE
across restarts via a local SQLite file (`BRAIN_DB`), written through on every
mutation and restored at startup, 100% local with no server or API).
...
10. **Does not persist by default.** Out of the box it uses in-memory repositories;
    ...
    (`app.ts:72`, `db/repositories.ts:220-238`). The **entire Company Brain** —
    all six scalar stores plus the experience/pattern/graph sub-brains — is durable
    when `BRAIN_DB` (a local SQLite file) is set (Sprint 6, slices 1–3); Executive
    Memory and the Decision Journal remain RAM-only for now.
