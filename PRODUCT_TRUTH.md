...
before enforcement; the Company Brain's scalar sub-brains — marketing (which
grounds the governance observe chain), creative, SOP, sales, DNA, and brand — are
now DURABLE across restarts via a local SQLite file (`BRAIN_DB`), written through
on every enrichment and restored at startup, 100% local with no server or API).
...
10. **Does not persist by default.** Out of the box it uses in-memory repositories;
    ...
    (`app.ts:72`, `db/repositories.ts:220-238`). The Company Brain's **six scalar
    stores** (marketing, creative, SOP, sales, DNA, brand) are durable when
    `BRAIN_DB` (a local SQLite file) is set (Sprint 6, slices 1–2); the three
    port-backed sub-brains (experience, patterns, graph) remain RAM-only for now.
...
| **Durable Company Brain — every scalar sub-brain survives restarts (Sprint 6, slices 1–2)** — all six scalar stores — **marketing** (grounds the governance observe chain), **creative, SOP, sales, DNA, brand** — are now **written through to a local SQLite file** on every enrichment/set (the already-merged long-run average, not the raw last sample) and **restored on startup**, so learned knowledge is no longer wiped by a restart. Uniform `(k, data)` key-value tables behind one `BrainStore` port. 100% local (`node:sqlite`, a file), no server, no API. Opt-in via `BRAIN_DB`; a plain `new App()` stays pure in-memory (unchanged for dev/tests). A transparent decorator: reads, sub-brains, and the sample-weighted merge are unchanged — it just also survives a restart | `apps/web/src/brain-persistence.ts` (`PersistentCompanyBrain` decorator, `SqlBrainStore` over `@ados/persistence` `SqliteDatabase`, `isRestorableBrain`), restore seams `domains/company-brain/.../in-memory-company-brain.ts` (`restoreMarketing/Creative/Sop/Sales/Dna/Brand`, no re-merge), injected `apps/web/src/app.ts` (ctor + `start()` restore) + wired `main.ts` (`BRAIN_DB`), `brain-persistence.test.ts` (6: write-through, cross-instance restore of all scalar stores on a real file, merged-not-raw for marketing + SOP, delegation) + `brain-persistence.app.test.ts` (2: App restores on start; default App does not persist). **Still 🔶/❌ (later slices):** the three port-backed sub-brains (experience/patterns/graph) are still RAM-only; archive/compaction; per-tenant brain isolation (pre-existing global-brain gap, unchanged here) |
