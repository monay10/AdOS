...
block); the offline generation path now flows through the governed AIManager runtime
— routing → resilient inference pipeline → format/validate → monitoring — with
byte-for-byte identical output, replacing the thin bypass on that path).
...
| **Governed inference runtime is the generation path (offline)** — the app's generation no longer calls the thin bypass; it flows through the governed `AIManager` runtime (`ai-manager/.../runtime/manager.ts`): capability routing (`CapabilityRouter`) → resilient `InferencePipeline` (retry / fallback / circuit-breaker) → `JsonResponseFormatter` → validation → `InMemoryMonitoring` → sealed job. **Output and model name are byte-for-byte identical** — the inference engine reuses the exact `OfflineAIManager` responder and a single `offline-deterministic` model. Governance stages (evidence/confidence/constitution) are **not** wired into this runtime — they stay as the StagedAIManager observe stages + approval-gate advisory, unchanged. This is the engine swap, not a behavior change | `apps/web/src/governed-inference.ts` (`createOfflineGovernedManager`, `OfflineInferenceEngine`, request-echo context, pass-through validation), wired `apps/web/src/app.ts` (App default), full web suite (119) passes byte-identical through it. **Still 🔶/❌:** the LIVE (local-model) path still uses `LiveAIManager` (4.4b); governance stages inside the governed runtime remain off (later rungs) |
