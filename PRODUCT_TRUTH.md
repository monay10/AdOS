...
before enforcement; the Company Brain's marketing memory — the store that grounds
the governance observe chain — is now DURABLE across restarts via a local SQLite
file (`BRAIN_DB`), written through on every enrichment and restored at startup,
100% local with no server or API).
...
10. **Does not persist by default.** Out of the box it uses in-memory repositories;
    ...
    (`app.ts:72`, `db/repositories.ts:220-238`). The Company Brain's **marketing**
    memory is durable when `BRAIN_DB` (a local SQLite file) is set (Sprint 6,
    slice 1); the brain's other stores remain RAM-only for now.
...
| **Durable Company Brain — marketing memory survives restarts (Sprint 6, slice 1)** — the per-vertical `MarketingInsight` store (the exact store that grounds the governance observe chain) is now **written through to a local SQLite file on every enrichment** — as the already-merged long-run average, not the raw last sample — and **restored on startup**, so learned performance is no longer wiped by a restart. 100% local (`node:sqlite`, a file), no server, no API. Opt-in via `BRAIN_DB`; a plain `new App()` stays pure in-memory (unchanged for dev/tests). A transparent decorator: reads, sub-brains, and the sample-weighted merge are unchanged — it just also survives a restart | `apps/web/src/brain-persistence.ts` (`PersistentCompanyBrain` decorator, `SqlBrainStore` over `@ados/persistence` `SqliteDatabase`, `isRestorableBrain`), restore seam `domains/company-brain/.../in-memory-company-brain.ts` (`restoreMarketing`, no re-merge), injected `apps/web/src/app.ts` (ctor + `start()` restore) + wired `main.ts` (`BRAIN_DB`), `brain-persistence.test.ts` (4: write-through, cross-instance restore on a real file, merged-not-raw, delegation) + `brain-persistence.app.test.ts` (2: App restores on start; default App does not persist). **Still 🔶/❌ (later slices):** the brain's other stores (creative/sop/sales/dna/brand) + sub-brains (experience/patterns/graph) are still RAM-only; archive/compaction; per-tenant brain isolation (pre-existing global-brain gap, unchanged here) |
